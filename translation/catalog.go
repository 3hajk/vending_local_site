...
func GetLocale(lang string) *message.Printer {
	dict, err := parseYAMLDict()
	if err != nil {
	...
	switch lang {
	case "ru":
		logger.Info("Locale is Russian")
		return message.NewPrinter(language.Russian)
	case "ua":
		logger.Info("Locale is Ukrainian")
		return message.NewPrinter(language.Ukrainian)
	default:
